...
def saveDetails():
    ...
    if request.method == "POST":
        try:
            ...
            sql = """INSERT into tblbook(title, isbn, price, quantity, total_pages, publication_date, genre_id, language_id, book_cover, instock)
            values (% s, % s, % s, % s, % s, % s, % s, % s, % s, % s)"""
            # add the form variables for each column
            val = (title, isbn, price, qty, numpages,
                   dt, genre, lang, full_filename, instk)
            cur.execute(sql, val)
            con.commit()
...
def searchbook():
    ...
    try:
        sql = "select * from tblbook bk INNER JOIN tbllanguage lg ON \
        bk.language_id=lg.language_id WHERE language_name LIKE %s ORDER BY title DESC"
        val = ('%' + lang + '%',)
        cur.execute(sql, val)
        rows = cur.fetchall()
